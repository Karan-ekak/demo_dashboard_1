...
from folder_path_name import LOGS_PATH
"""
Defining A Loggine
"""
logger.add(f"{LOGS_PATH}Database_Argus.log", level="INFO", rotation="100 MB")
...
class ArgusDB:
    ...
    def create_connection(self):
        """
        Creating A Connection with PostGreSQL to Perform Operation from Database.

        Return:
        ==> If Connection is Created, return the connection object
        ...
        try:
            ...
            logger.debug(f"Issue in Creating Connection: {e}")
    ...
    def get_query(self, query):
        ...
        try:
            ...
            logger.debug(f"Table does not exist: {e}")
        except Exception as e:
    ...
    def update_query_where(self, table_name: str, columns, where_condition: str):
        """Updates a PostgreSQL table based on the given columns and where condition.
            ...
            query : ArgusDB().update_query_where(table_name='1002_users',columns={ 'mobile' : '9876543210'},where_condition="user_id=5044")
            Args:
    ...
    def get_query_dict(self, query, connection=None, cursor=None):
        """
        Executes a query and returns the result as a list of dictionaries.
        If connection and cursor are provided, reuse them.
        """
        ...
        try:
            ...
            if new_connection and cursor:
                cursor.close()
            if new_connection and connection:
                connection.close()
    ...
    def fetch_query(self, query: str, params: tuple = ()) -> list:
        ...
        Parameters:
        - query: str - The SQL query to execute (use placeholders: %s for PostgreSQL).
        - params: tuple - Parameters to bind to the SQL query.

        Returns:
        - List of rows (as tuples) or an empty list if none found or on error.
        """
        connection = self.create_connection()
        if not connection:
            return []
        ...
        try:
            ...
            logger.debug(f"Error executing fetch query: {e}")
    ...
    def execute_query(self, query: str, params: tuple = ()) -> bool:
        """
        Executes an arbitrary SQL query (INSERT, UPDATE, DELETE, etc.).
        ...
        """
        connection = self.create_connection()
        connection.autocommit = True  # Enable autocommit mode to avoid transaction management issues
        q_status = False  # Query status
        ...
        if connection != False:
            try:
                ...
                logger.debug(f"The error '{e}' occurred during query execution")
                logger.exception("Operational Error during query execution")
            except Exception as Err:
                logger.debug(f"Issue: {Err}")
    ...
    def get_all(self, query):
        ...
        if connection != False:
            ...
            try:
                ...
                logger.debug(f"Table is Not Exist '{e}' occurred")
            except Exception as Err:
                logger.debug(F"Issue {Err}")
        return q_status
    ...
    def get_columns(self, table_name: str, columns: list, where: str = "", getOne=False, extra=''):
        ...
        if where:
            query += f' WHERE {where}'
        if getOne:
            return self.get_query(query+extra)
    ...
    def get_all_unknown(self, customer_id,offset=None):
        """
        Show all the data from table query
        ...
        if connection != False:
            ...
            try:
                ...
                query=" "
                if offset:
                    query = F'select photo_url,name,date,time,argus_id from \"argus\".\"{customer_id}_attendance_entries\" where \"name\" like \'unknow%\' ORDER BY date desc OFFSET {offset} Limit 10 ;'
                else:
                    query = F'select photo_url,name,date,time,argus_id from \"argus\".\"{customer_id}_attendance_entries\" where \"name\" like \'unknow%\' ORDER BY date desc  Limit 10 ;'
                print(query)
                cursor.execute(query)
                result = cursor.fetchall()
                print(result)
    ...
    def get_all_incident(self, customer_id,offset=None,where:dict= None):
        ...
        try:
            ...
            if where :
                ...
                query =F'SELECT priority,incident_id,category,date,assignee,location,status,assigned_by,time,camera_id FROM argus.\"{table_name}\" {where_condition} ORDER BY incident_id DESC LIMIT 100 ; '
            elif offset:
                query = F'SELECT priority,incident_id,category,date,assignee,location,status,assigned_by,time,camera_id FROM argus.\"{table_name}\" ORDER BY incident_id DESC OFFSET {offset} LIMIT 100 ; '
            else:
                query = F'SELECT priority,incident_id,category,date,assignee,location,status,assigned_by,time,camera_id FROM argus.\"{table_name}\" ORDER BY incident_id DESC LIMIT 100 ; '
    ...
    def get_incidents_with_filters(self, customer_id, selected_dates=None, selected_start_time=None, selected_end_time=None, limit=100, offset=0):
        try:
            table_name = f"{customer_id}_cam3_material_count"
            base_query = f"SELECT id, video_id, total_material_count, video_timestamp FROM argus.\"{table_name}\""
            conditions = []
            ...
            if selected_start_time:
                conditions.append(f"TO_CHAR(video_timestamp, 'HH24:MI:SS') >= '{selected_start_time}'")
            if selected_end_time:
                conditions.append(f"TO_CHAR(video_timestamp, 'HH24:MI:SS') <= '{selected_end_time}'")
            ...
            return self.get_all(base_query)
    ...
    def get_10_incident3(self, customer_id, selected_dates=None, selected_start_time=None, selected_end_time=None, limit=100, offset=0):
        try:
            table_name = f"{customer_id}_cam3_defect_count"
            base_query = f"SELECT id, video_id, total_defect_count, video_timestamp FROM argus.\"{table_name}\""
            conditions = []
    ...
    def get_10_incident_breakdown(self, customer_id, selected_dates=None, selected_start_time=None, selected_end_time=None, limit=100, offset=0):
        try:
            table_name = f"{customer_id}_cam3_material_count"
            base_query = f'SELECT id, video_id, total_material_count, video_timestamp FROM argus.\"{table_name}\"'
    ...
    def get_one_incident(self, customer_id, incidentID):
        """Get all priority,incident_id,category,date,assignee,location,status,assigned_by
         ...
         """
        try:
            table_name = "1007_cam3_material_count"
            logger.info(f'_Get id,video_id,total_material_count,video_timestamp Where Table: {table_name}')
            query = f'SELECT id,video_id,total_material_count,video_timestamp FROM argus.\"{table_name}\" WHERE video_id={incidentID};'
            return self.get_query(query)
        except Exception as E:
            logger.exception("Error in user Email")
            return False
        
    def today_breakdown(self, incidentID):
        try:
            query = """
                SELECT COUNT(*) AS zero_material_count
                FROM argus."1007_cam3_material_count"
                WHERE total_material_count = 0
                AND DATE(video_timestamp) = CURRENT_DATE
                AND (
                    (EXTRACT(HOUR FROM video_timestamp) BETWEEN 6 AND 13 AND NOT (video_timestamp::time BETWEEN '09:00:00' AND '09:30:00'))
                    OR
                    (EXTRACT(HOUR FROM video_timestamp) BETWEEN 14 AND 21 AND NOT (video_timestamp::time BETWEEN '17:00:00' AND '17:30:00'))
                    OR
                    (
                        ((EXTRACT(HOUR FROM video_timestamp) BETWEEN 22 AND 23) OR (EXTRACT(HOUR FROM video_timestamp) BETWEEN 0 AND 5))
                        AND NOT (video_timestamp::time BETWEEN '02:00:00' AND '02:30:00')
                    )
                )
            """
            result = self.get_query_dict(query)
            zero_material_count = result[0]['zero_material_count'] if result and result[0]['zero_material_count'] is not None else 0
            hours, minutes = divmod(zero_material_count, 60)
            return f"{hours} h {minutes} m"
        except Exception as E:
            logger.exception("Error in fetching breakdown data")
            return False

   
    

    def get_today_defect_count(self, incidentID):
        try:
            connection = self.create_connection()
            connection.autocommit = True
            cursor = connection.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
            ...
            FROM argus."1007_cam3_defect_count"
            WHERE video_timestamp >= CURRENT_DATE
            AND video_timestamp < CURRENT_DATE + INTERVAL '1 day';
            '''
            result = self.get_query_dict(query, connection, cursor)
            total_defects = result[0]['total_defects_today'] if result and result[0]['total_defects_today'] is not None else 0
            print("QUERY : ",query)
    ...
    def today_breakdown_hour(self, incidentID):
        try:
            query = """
                SELECT 
                    date_trunc('hour', video_timestamp) AS hour,
                    count(*) AS material_sum
                FROM argus."1007_cam3_material_count"
                WHERE total_material_count = 0 AND DATE(video_timestamp) = CURRENT_DATE
                GROUP BY hour
                ORDER BY hour DESC;
            """
            result_hour = self.get_query_dict(query)
            return [{"hour": row["hour"].isoformat(), "count": row["material_sum"]} for row in result_hour]
        except Exception as E:
            logger.exception("Error in fetching hourly breakdown data")
            return []

    def today_defects_hour(self, incidentID):
        try:
            query = """
                SELECT 
                    date_trunc('hour', video_timestamp) AS hour,
                ...
                FROM argus."1007_cam3_defect_count"
                WHERE total_defect_count != 0 AND DATE(video_timestamp) = CURRENT_DATE
                GROUP BY hour
                ORDER BY hour DESC;
            """
            result_hour = self.get_query_dict(query)
            return [{"hour": row["hour"].isoformat(), "count": row["total_defects"]} for row in result_hour]
        except Exception as E:
            logger.exception("Error in fetching hourly defect data")
            return []



    def today_material_hour(self, incidentID):
        try:
            query = """
                SELECT 
                    date_trunc('hour', video_timestamp) AS hour,
                ...
                FROM argus."1007_cam3_material_count"
                WHERE DATE(video_timestamp) = CURRENT_DATE
                GROUP BY hour
                ORDER BY hour DESC;
            """
            result_hour = self.get_query_dict(query)
            return [{"hour": row["hour"].isoformat(), "count": row["total_material_count"]} for row in result_hour]
        except Exception as E:
    ...
    def get_quarterly_data(self, table_name, column_name):
        try:
            query = f"""
                SELECT 
                    date_trunc('hour', video_timestamp) + INTERVAL '15 min' * FLOOR(EXTRACT(minute FROM video_timestamp) / 15) AS quarter,
                    SUM(COALESCE({column_name}, 0)) AS total_count
                FROM argus."{table_name}"
                WHERE 
                    DATE(video_timestamp) = CURRENT_DATE
                    AND NOT (
                        (video_timestamp::time >= TIME '06:00' AND video_timestamp::time < TIME '14:00' AND video_timestamp::time >= TIME '09:00' AND video_timestamp::time < TIME '09:30')
                        OR (video_timestamp::time >= TIME '14:00' AND video_timestamp::time < TIME '22:00' AND video_timestamp::time >= TIME '18:00' AND video_timestamp::time < TIME '18:30')
                        OR ((video_timestamp::time >= TIME '22:00' OR video_timestamp::time < TIME '06:00') AND video_timestamp::time >= TIME '02:00' AND video_timestamp::time < TIME '02:30')
                    )
                GROUP BY quarter
                ORDER BY quarter DESC;
            """
            results = self.get_query_dict(query)
            return [{"quarter": row["quarter"].isoformat(), "count": row["total_count"]} for row in results]
        except Exception as E:
    ...
    def today_defects_quarter(self, incidentID):
        return self.get_quarterly_data("1007_cam3_defect_count", "total_defect_count")
    

    def today_material_quarter(self, incidentID):
        return self.get_quarterly_data("1007_cam3_material_count", "total_material_count")
    ...
    def get_today_incident(self, customer_id, date):
        try:
            table_name = f"{customer_id}_incident"
            logger.info(f'_Get incident data from table: {table_name}')
            query = f'''
                SELECT priority, incident_id, category, date, assignee, location, status, assigned_by 
                FROM argus."{table_name}" 
                WHERE date = '{date}';
            '''
            return self.get_query(query)
        except Exception as E:
            logger.exception("Error in fetching incident data")
            return False 
        
    def fetch_json_column(self, table_name, column_name, incidentID):
        try:
            logger.info(f'_Get {column_name} from Table: {table_name}')
            query = f'SELECT {column_name} FROM argus."{table_name}" WHERE video_id={incidentID};'
    ...
    def fetch_one(self, incidentID):
        return self.fetch_json_column("1007_cam3_material_count", "material_json", incidentID)
    
    def fetch_one2(self, incidentID):
        return self.fetch_json_column("1007_cam3_defect_count", "defect_material_json", incidentID)



    def fetch_video_timestamp(self, table_name, incidentID):
        try:
            logger.info(f'_Get video_timestamp from table: {table_name}')
            query = f'SELECT video_timestamp FROM argus."{table_name}" WHERE video_id={incidentID};'
    ...
    def fetch_time(self, incidentID):
        return self.fetch_video_timestamp("1007_cam3_material_count", incidentID) 



    def fetch_time2(self, incidentID):
        return self.fetch_video_timestamp("1007_cam3_defect_count", incidentID)




    def insert_new_incident(self, customer_id, incident_id, priority, category, date, location, status, user_id,
                            assignee,video_link,mid,comment=''):
        ...
        try:
            ...
            query = F'insert into "argus"."{table_name}" (incident_id,priority,category,date,location,status,assigned_by,assignee,video_link,mid) values ({incident_id},{priority},{category},\'{date}\',{location},{status},{user_id},{assignee},\'{video_link}\',{mid}) ; '
            ...
            if q_status:
                ...
                if comment:
                    arg = (incident_id, '{ \"%s\": { "status": %d, "assigned_by": %d, "assignee": %d ,"comment" : \"%s\" }}' % (
                    date, int(status), int(user_id), int(assignee),str(comment)))
                    #print(query2, arg)
    ...
    def update_incident_history(self, customer_id, date, status, assignee, assigned_by, comment, incident_id):
        ...
        if connection != False:
            try:
                ...
                cursor.execute("""
                UPDATE argus."%s"
                SET incident_history = jsonb_set(incident_history, '{%s}', '{"status": %d,"assignee":%d , "assigned_by" :%d , "comment":"%s"}') ,
                    status = %d ,assignee = %d
                WHERE incident_id = %d ;
                """ % (table_name, date, status, assignee, assigned_by, comment, status,assignee,incident_id))
                connection.commit()
                q_status = True
                #print("Query executed successfully")
                logger.info("Successfully Execute Query")
    ...
    def update_assignedOfficer_incident(self, customer_id, incident_id, assignee):
        ...
        try:
            ...
            logger.exception(F"Error in Assigned Incident {E}")
            #print(E)
            return False
    ...
    def update_user_account_detail(self, customer_id, user_email, user_id, user_name, user_role):
        ...
        if connection != False:
            try:
                ...
                # print(er)
                logger.exception(F"Error is {er}")
                return False
        return q_status
    ...
    def insert_db(self, table_name, Key_values):
        try:
            ...
            print("QUERY for insertion",query)
            return self.insert_query(query)
        except Exception as err:
            # print(err)
    ...
    def generate_incidentID(self,customer_id):
        ...
        today_date=ist.ist_time().strftime("%y%m%d%H%M")


        latest_IncidentID=self.get_ca_latest_incident(customer_id)
    ...
    def get_entry_exit_time_day(self, argus_id:str,day:str):
        """
        Show all the Team Member Entry and Exit day Wise from table query
        ...
        if connection != False:
            result = None
            cursor = connection.cursor()
            entry_time=day+'_entry_time'
            exit_time=day+'_exit_time'
            try:
                # This line look horror , Purpose for POSTGRE Wants Values must be Dual Quotes
                # Its Give a QUotes Using "ESCAPE SEQUENCE"
                query = F'select \"emp_name\",\"{entry_time}\",\"{exit_time}\",\"entry_time_threshold\",\"exit_time_threshold\"  from \"argus\".\"{argus_id}_team\" where alert_switch=true and \"{entry_time}\" not like \'-1\' ; '
    ...
    def get_name_time_present_today(self, argus_id,today_date):
        """
        Show all the Team Member data from table query
        ...
        if connection != False:
            ...
            try:
                # This line look horror , Purpose for POSTGRE Wants Values must be Dual Quotes
                # Its Give a QUotes Using "ESCAPE SEQUENCE"
                # query = F'select \"emp_name\",\"entry_time\",\"exit_time\",\"entry_time_threshold\",\"exit_time_threshold\"  from \"argus\".\"{argus_id}_team\" where alert_switch=true  '

                query = F'select \"name\",\"time\" from \"argus\".\"{argus_id}_attendance_entries\" where date=\'{today_date}\' and  \"name\" not like \'unknow%\' ORDER BY time ASC ;'
                cursor.execute(query)
                result = cursor.fetchall()
    ...
    def get_report_id_static_csv(self, argus_id):
        ...
        if connection != False:
            ...
            try:
                ...
                logger.exception("Issue is")
        return q_status
    def get_shift_materials_breakdown(self, incidentID):
        try:
            query = """
                WITH shifts AS (
                    SELECT 'Shift A' AS shift, '06:00:00'::time AS start_time, '09:00:00'::time AS lunch_start, '09:30:00'::time AS lunch_end, '14:00:00'::time AS end_time
                    UNION ALL
                ...
                SELECT 
                    s.shift,
                    FLOOR(COUNT(*) / 60) || 'h ' || (COUNT(*) % 60) || 'm' AS zero_material_duration
                FROM 
                    argus."1007_cam3_material_count" m
                JOIN 
                    shifts s 
                    ON (
                        (m.video_timestamp::time >= s.start_time AND m.video_timestamp::time <= s.end_time)
                        AND NOT (m.video_timestamp::time BETWEEN s.lunch_start AND s.lunch_end)
                    )
                WHERE 
                    DATE(m.video_timestamp) = CURRENT_DATE
    ...
    def get_entry_exit_thershold_time(self, argus_id):
        """
        Show all the Team Member data from table query
...
if __name__ == '__main__':
    pass   
    table=F''' argus."WorkOrder" '''
    colums=('created_at','WorkOrderId',)
    extra=' LIMIT 10'
    data=ArgusDB().get_columns(table_name=table,columns=colums,extra=extra)
    print(data)